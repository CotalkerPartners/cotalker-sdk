...
export default class Answer {
	...
	private constructor(private cotAnswer: COTAnswer) {
	...
	getString(identifier: string): string {
		return (
			this.cotAnswer.data.find(d => d.identifier === identifier)?.process[0] ??
			""
		);
	}
	...
	getProcess(identifier: string): string[] {
		return (
			this.cotAnswer.data.find(d => d.identifier === identifier)?.process ?? []
		);
	}

	getPropertyResponse<T extends COTProperty>(identifier: string): T | null {
		try {
			const resp = this.cotAnswer.data.find(d => d.identifier === identifier)
				?.responses[0];
