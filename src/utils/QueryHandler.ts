...
export class QueryHandler<T> {
	...
	public async getQuery(query): Promise<CotalkerQueryAPIResponse<T>> {
		...
		const invalidEntry = queryEntries.find(([_, v]) =>
			isInvalidQueryValue(v)
		);
		if (invalidEntry) {
			console.info(
				`Invalid empty query value for key ${invalidEntry[0]} in GET /${this.entity}`
			);
		}
		const response = (
			await this.instance.get(
				`/api/v2/${this.entity}?${querystring.encode(query)}`
			)
		)?.data;
