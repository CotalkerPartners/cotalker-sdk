...
export default class COTTaskClient {
	...
	public async getTask<T extends COTTask>(
		...
		const task = await this._instance.get<T>(
	...
	public async getTaskBySerial(
		...
		const task = await this._instance.get<COTTask>(
	...
	public async queryTasksFilter(
		...
		const task = await this._instance.get<FilteredTasks[]>(
	...
	public async patchTask(
		...
		const task = await this._instance.patch<COTTask>(
			`/api/tasks/${taskGroupId}/task/${taskId}`,
			body
		);
		return task;
	...
	public async findTasks<T extends COTTask>(
		...
		const task = await this._instance.post<T[]>(
	...
	public async patchMultiTasks(
		...
		const task = await this._instance.post<COTTask[]>(
